function saveCellData(cell_id,nrn_model_ver,iontype,cell_model)
    if nargin == 0
       cell_id = 1; 
       nrn_model_ver = 'maxH';
       iontype = 'Default';
    end
    if nargin < 4 % no cell_model input    
    ...
    % load（areas1.txt, branchorders1.txt, coordinates1.txt, diams1.txt, parentnames1.txt, secnames1.txt, sectypes1.txt）
    if ispc
        system(['D:\nrn\bin\nrniv.exe -nobanner ' options nrn_file commands]);              % windows command, Note: 'D:\nrn\bin\nrniv.exe'
    elseif isunix       
        system(['./special ' options nrn_file commands]);                                   % Linux or Apple maxOS
    end
    ...
    nrn_model_ver_fold = fullfile(mat_dir,'cell_data', sprintf('%s_%s', nrn_model_ver, iontype));
    ...
    fclose(fid);
    secnames = secnames{1};     
    secnames{1} = [secnames{1} '.v(0.50)'];                                                 % add to conform to format, for bNAC219_L1_NGCDA_095997ce92[0].soma[0]
    ...
    basal_comps = 5*cellfun(@(x) ~isempty(x),strfind(secnames,'dend'));                     % basal dend
    apical_comps = 6*cellfun(@(x) ~isempty(x),strfind(secnames,'apic'));                    % apic dend
    ...
    %% Save sectypes
    sectypes = readmatrix(sectypes_file); 
    ...
    %% Save branchorders
    branchorders = readmatrix(branchorders_file);
