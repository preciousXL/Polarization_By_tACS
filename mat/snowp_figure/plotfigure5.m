...
for i=1:25
    ...
    datapython = struct();
    polar_soma = cellfun(@(x) x(idxsoma_sec), polarize_freq);
    polar_axon = cellfun(@(x) x(idxaxon_sec), polarize_freq);
    polar_basal = cellfun(@(x) x(idxbasal_sec), polarize_freq);
    datapython.polar_soma = polar_soma;
    datapython.polar_axon = polar_axon;
    datapython.polar_basal = polar_basal;
    % plot
    if idxapic_sec ~= 0
        polar_apic = cellfun(@(x) x(idxapic_sec), polarize_freq);
        datapython.polar_apic = polar_apic;
    ...
    xlim([0, 100]); 
    ylim([0, inf]);
    data_figure_ampfreq{i, 1} = datapython;
end
t.TileSpacing = 'compact';
t.Padding = 'compact';
%% save data for python plotfigures
savefile = fullfile(mat_dir, 'snowp_data', sprintf('Data_Figure_AmpFreq%s.mat', biophysics));
save(savefile, "data_figure_ampfreq", "list_f");
